...
template <class T, class Compare>
int quick_sort(Array<T> &arr, Compare cmp, int from, int amount) {
  int part = partition(arr, cmp, from, amount);
  if (part > 0)
    quick_sort(arr, cmp, from, part);
  if (part + 1 < amount)
    quick_sort(arr, cmp, from + part + 1, amount - (part + 1));
}

int find_longest_painted_line(Array<PaintDifferential> &arr,
                              int target_height) {
