...
class HashTable {
public:
    HashTable(int coef = DEFAULT_HASH_FUNCTION_COEF, int maxSize = DEFAULT_HASH_TABLE_SIZE);
...
private:
    ...
    //h(k) не меняется, => сокращаем число вычислений, не вычисляя хеш для каждого 
...
template<class T>
HashTable<T>::HashTable(int coef, int maxSize) : table(nullptr), deleted(nullptr), hash_function_coef(coef), maxSize(maxSize), keysCount(0) {
    table = new T*[maxSize];
...
bool HashTable<T>::Add(const T& key) {
    ...
    while (table[hash] != nullptr || deleted[hash]) {
        ...
        if (hash == first_hash) {
            // Если мы прошли по всей таблице и не нашли свободного места, добавление провалилось
            return false;
        }
    }
...
template<>
int HashTable<std::string>::_Hash(const std::string& key) const {
    int hash = 0;
    for (auto& c : key)
...
template<class T>
int HashTable<T>::_Has(const T& key) const {
    if (keysCount == 0) 
        return -1;
...
template<class T>
double HashTable<T>::_Coef() const {
    return double(keysCount) / double(maxSize);
}

template<class T>
void HashTable<T>::_GrowTable() {
