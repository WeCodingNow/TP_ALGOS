...
template<class T>
struct BinarySearchNode : public BinaryNode<T> {
    BinarySearchNode();
    ~BinarySearchNode();
...
template<class T>
struct TreapNode : public BinaryNode<T> {
    TreapNode();
    ~TreapNode();
...
template<class T>
class Treap : public BinaryTree<T> {
public:
    Treap();
    ~Treap();
...
private:
    ...
    void _Split(TreapNode<T>* node, int key, TreapNode<T>*& left, TreapNode<T>*& right);
...
template<class T>
void AddToBinarySearchTree(BinarySearchTree<T>& bst, std::vector<std::pair<T, int>> const& vec);
...
void TraverseBreadthFirst(BinaryTree<T>& bt, Func f, bool layerInfo) {
    ...
    while (!deque.empty()) {
        BinaryNode<T>* node = deque.front(); deque.pop_front();
        f(node);
        if (!node) {
            if (deque.empty())
                break;
...
template<class T>
void Print<T>::operator()(BinaryNode<T>*& arg) {
    if (!arg) {
        std::cout << "| ";
        return;
    }
...
template<class T>
MaxLayer<T>::MaxLayer(int* ptr) : returnValue(ptr), currentLayer(0), maxLayer(0) {
}
...
template<class T>
void MaxLayer<T>::operator()(BinaryNode<T>*& arg) {
    if (!arg) {
...
template<class T>
BinaryNode<T>* BinarySearchNode<T>::getLeft() {
    return left;
}

template<class T>
BinaryNode<T>* BinarySearchNode<T>::getRight() {
    return right;
}

template<class T>
T& BinarySearchNode<T>::getData() {
    return data;
}
...
template<class T>
bool BinarySearchTree<T>::Has(const T& key) {
    return (_Find(key));
}
...
template<class T>
BinaryNode<T>* TreapNode<T>::getLeft() {
    return left;
}

template<class T>
BinaryNode<T>* TreapNode<T>::getRight() {
    return right;
}

template<class T>
T& TreapNode<T>::getData() {
    return data;
}
...
template<class T>
BinaryNode<T>* Treap<T>::getRoot() {
    return root;
}

template<class T>
void Treap<T>::_Split(TreapNode<T>* node, int key,
...
template<class T>
void Treap<T>::Add(const T& key, const int& priority) {
    if (!root) {
